--[[
    LunarSpy v2.2 SOURCE

    LunarSpy is a lightweight penetration testing tool that logs remote calls.
...
-- shuts down the previous instance of LunarSpy
if _G.LunarSpyExecuted and type(_G.LunarSpyShutdown) == "function" then
	print(pcall(_G.LunarSpyShutdown))
end
...
local Highlight =
	loadstring(
		game:HttpGet("https://github.com/exxtremestuffs/LunarSpySource/raw/master/highlight.lua")
	)()
...
local LunarSpy2 = Instance.new("ScreenGui")
...
LunarSpy2.Name = "LunarSpy2"
LunarSpy2.ResetOnSpawn = false

Background.Name = "Background"
Background.Parent = LunarSpy2
...
_G.LunarSpyCONFIG_MaxRemotes = 500
...
-- whether or not LunarSpy uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable LunarSpy's keyToString for remotes
...
--- @param args any[]
--- @return string
function LunarSpy:ArgsToString(method, args)
...
--- @param t any[]
--- @return string
function LunarSpy:TableToVars(t)
...
--- @param value any
--- @return string
function LunarSpy:ValueToVar(value, variablename)
...
--- @param value any
--- @return string
function LunarSpy:ValueToString(value)
	return v2s(value)
end

--- Generates the LunarSpy function info
--- @param func function
--- @return string
function LunarSpy:GetFunctionInfo(func)
...
function LunarSpy:GetRemoteFiredSignal(remote)
...
--- @param remote Instance
--- @param f function
function LunarSpy:HookRemote(remote, f)
	assert(typeof(remote) == "Instance", "Instance expected, got " .. typeof(remote))
...
--- @param remote any
function LunarSpy:BlockRemote(remote)
	assert(
...
--- @param remote any
function LunarSpy:ExcludeRemote(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
...
--- Prevents remote spam from causing lag (clears logs after `_G.LunarSpyCONFIG_MaxRemotes` or 500 remotes)
function clean()
	local max = _G.LunarSpyCONFIG_MaxRemotes
...
--- Executed when the toggle button (the LunarSpy logo) is hovered over
...
--- Called when mouse enters LunarSpy
function mouseEntered()
	local existingCursor = LunarSpy2:FindFirstChild("Cursor")
	while existingCursor do
		existingCursor:Destroy()
		existingCursor = LunarSpy2:FindFirstChild("Cursor")
	end
	...
	customCursor.Image = ""
	customCursor.Parent = LunarSpy2
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.ForceHide
	RunService:BindToRenderStep("LunarSpy_CURSOR", 1, function()
		if mouseInGui and _G.LunarSpyExecuted then
			...
			RunService:UnbindFromRenderStep("LunarSpy_CURSOR")
		end
	end)
end
...
function backgroundUserInput(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 and inResizeRange then
		...
		RunService:BindToRenderStep("LunarSpy_RESIZE", 1, function()
		...
		table.insert(
			...
			UserInputService.InputEnded:Connect(function(inputE)
				if input == inputE then
					RunService:UnbindFromRenderStep("LunarSpy_RESIZE")
				end
			end)
		)
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY LunarSpy\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function genScript(remote, args)
	...
	gen = "-- Script generated by LunarSpy - credits to exx#9394\n\n" .. gen
	prevTables = {}
	return gen
end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.LunarSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.LunarSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in LunarSpy:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.LunarSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("LunarSpy: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
function shutdown()
	...
	_G.LunarSpyExecuted = false
end

-- main
if not _G.LunarSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("LunarSpy cannot run on the server!")
		end
		if
			not hookfunction
		...
		then
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running LunarSpy)\nMissing: "
					.. table.concat(missing, ", ")
			)
		end
		_G.LunarSpyShutdown = shutdown
		...
		if syn and syn.protect_gui then
			pcall(syn.protect_gui, LunarSpy2)
		end
		bringBackOnResize()
		LunarSpy2.Parent = --[[gethui and gethui() or]]
			CoreGui
		_G.LunarSpyExecuted = true
	...
	if not succeeded then
		warn(
			"A fatal error has occured, LunarSpy was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		LunarSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Get Script", function()
	...
	if selected then
		setclipboard(LunarSpy:ValueToString(selected.Source))
		TextLabel.Text = "Done!"
	end
end)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the LunarSpy serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the LunarSpy Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes LunarSpy ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from LunarSpy logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up LunarSpy to detection and/or instability.",
...
newButton("GetReturnValue", function()
	...
	if selected then
		codebox:setRaw(LunarSpy:ValueToVar(selected.ReturnValue, "returnValue"))
	end
end)